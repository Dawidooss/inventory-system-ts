...
local function Server<T...>(self: Event<T...>): ServerEvent.Server<T...>
	...
	if not self.ServerEvent then
		self.ServerEvent = ServerEvent(self.Id, self.serializer, self.Validate, self.Unreliable)
	end
...
local function Client<T...>(self: Event<T...>): ClientEvent.Client<T...>
	...
	if not self.ClientEvent then
		self.ClientEvent = ClientEvent(self.Id, self.serializer, self.Unreliable) :: any
	end
...
local NoValidation = function(...)
	return ...
end

local function Event<T...>(Config: string | EventOptions, serializer): Event<T...>
